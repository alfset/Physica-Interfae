...
import { UNI_EXTENDED_LIST, UNI_LIST, UNSUPPORTED_LIST_URLS, PLANQTEST_LIST } from './lists'
...
class TokenSafetyLookupTable {
  ...
  createMap() {
    ...
    store.getState().lists.byUrl[PLANQTEST_LIST].current?.tokens.forEach((token) => {
